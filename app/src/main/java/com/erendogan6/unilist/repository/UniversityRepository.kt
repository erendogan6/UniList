...
@Singleton class UniversityRepository @Inject constructor(private val apiService: ApiService) {
    fun getProvincesStream(pagingConfig: PagingConfig = defaultPagingConfig()): Flow<PagingData<Province>> {
        return Pager(config = pagingConfig, pagingSourceFactory = { ProvincePagingSource(apiService) }).flow
    }

    private fun defaultPagingConfig() = PagingConfig(pageSize = 30, enablePlaceholders = true, maxSize = 90)
